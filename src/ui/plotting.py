...
def plot_constraints(constraints_str, objective_str=None, solution=None, padding=2):
    """Построение графика ограничений и ОДР с яркими границами и рисками"""
    ...
    # Собираем коэффициенты линий и точки пересечений
    lines = []  # (a, b, r, sign)
    intersections = []
    ...
    colors = ['#FF3333', '#33CC33', '#3333FF', '#FFCC33', '#CC33FF', '#33FFCC']

    # Оси
    ax.axhline(0, color='black', lw=1.5, alpha=0.4)
    ax.axvline(0, color='black', lw=1.5, alpha=0.4)

    # Рисуем ограничения с яркими линиями и рисками
    for idx, (a, b, r, sign) in enumerate(lines):
        ...
        if b != 0:
            y_line = (r - a*x_vals) / b
            ax.plot(x_vals, y_line, color=color, lw=3, label=label)
        else:
            x_line = r / a
            ax.axvline(x_line, color=color, lw=3, label=label)

        # Нормаль к линии и направление рисок
        norm = np.hypot(a, b)
        if norm == 0: continue
        ...
        for t in t_vals:
            ...
            ax.plot([x0, x1], [y0, y1], color=color, lw=1.2, alpha=0.8)
    ...
    # Убираем пунктирные линии уровня целевой функции
    # (раньше строились: ax.plot(... 'k--' ...))
    ...
    ax.set_title('Область допустимых решений (яркие границы)')
    plt.tight_layout()
    return fig
